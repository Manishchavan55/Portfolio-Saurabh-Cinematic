...
export function Certifications() {
  ...
  return <section id="certifications" className="section cert-section"><div className="wrap"><p className="kicker">04 / CAPABILITIES</p><h2 className="section-title">A ring of <em>proof.</em></h2><p className="section-copy">Drag the cylinder. Release it and inertia takes over. On touch devices, swipe across the cards.</p><div ref={ref} className="cylinder-stage" onPointerDown={e=>down(e.clientX)} onPointerMove={e=>move(e.clientX)} onPointerUp={up} onPointerCancel={up} onPointerLeave={up}>{cards.map(([tag,title,n],i)=>{const step=360/cards.length; return <div key={tag} className="cylinder-card" style={{ transform:`rotateY(${i*step}deg) translateZ(var(--radius))` }}><span className="card-no">{n}</span><b>{tag}</b><h3>{title}</h3><span className="card-line" /></div>})}<div className="cylinder" style={{transform:`translateZ(calc(var(--radius) * -1)) rotateY(${angle.current}deg)`}} /></div><div className="cert-footer"><span>08 NODES</span><span>360° ACCESS</span><span>DRAG / RELEASE</span></div></div></section>
}
